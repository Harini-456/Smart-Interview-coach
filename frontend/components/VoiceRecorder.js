...
export default function VoiceRecorder({ value, onChange, questionIndex }) {
  ...
  // Keep base in sync when question changes
  useEffect(() => {
  ...
  if (!isSupported) {
    return (
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5 text-center">
        <MicOff size={28} className="text-amber-500 mx-auto mb-2" />
        <p className="text-amber-500 font-medium text-sm">Voice not supported</p>
        <p className="text-muted-foreground text-xs mt-1">Your browser doesn't support Web Speech API. Try Chrome or Edge.</p>
        <textarea
          value={value || ""}
          onChange={(e) => onChange(e.target.value)}
          placeholder="Type your answer here instead..."
          rows={5}
          className="mt-4 w-full bg-background border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/40 resize-none"
        />
      </div>
    );
  }
